...
def run_inference(config_path: str, visible_path: str, output_path: str) -> None:
    ...
    # find all pngs in the visible directory
    images_path = glob.glob(f'{visible_path}/*.png') + glob.glob(f'{visible_path}/*.jpg') + glob.glob(f'{visible_path}/*.jpeg')

    for img_path in tqdm(images_path):
        # Resolve infrared path
        infrared_path = find_infrared_path(img_path)

        # Load images
        rgb = cv2.imread(img_path, cv2.IMREAD_UNCHANGED)
        ...
        if "segm" in config_path.lower():
            inference_engine.model.save_results(out_file, rgb, thermal, inference_engine.model.apply_colormap(results[0].full_image_segm))
            print("Segmentation mask shape:", results[0].full_image_segm.shape)
        elif "det" in config_path.lower():
            inference_engine.model.save_results(out_file, rgb, thermal, inference_engine.model.scale_draw_boxes(results[0].xywh, rgb.copy()))
...
if __name__ == "__main__":
    ...
    parser.add_argument("visible", help="Path to Visible (RGB) folder")
    parser.add_argument(
        "--out",
